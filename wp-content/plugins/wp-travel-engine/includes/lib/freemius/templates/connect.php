...
<script type="text/javascript">
	(function ($) {
		...
		if ( isNetworkActive ) {
			...
			$applyOnAllSites.on('click', function() {
			...
			$allSitesOptions.find( '.action' ).on('click',function( evt ) {
            ...
            if ( isNetworkUpgradeMode || hasAnyInstall ) {
                $skipActivationButton.on('click', function(){
                ...
                $delegateToSiteAdminsButton.on('click', function(){
		...
		if (requireLicenseKey) {
			...
			$marketingOptin.find( 'input' ).on('click', function() {
				$marketingOptin.removeClass( 'error' );
			});
		}
		//endregion
